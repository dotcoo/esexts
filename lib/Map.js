...
d(Map.prototype, {
  ...
  ...adapterMethods(Map.prototype, null, 't', 'set', 'delete'),
  ...
  getOrInsert$: function(key, defaultValue = null) {
    return this.has(key) ? this.get(key) : this.set$1(key, defaultValue);
  },
  getOrInsertComputed$: function(key, callback) {
    return this.has(key) ? this.get(key) : this.set$1(key, callback(key));
  },
  get$: function(key, defval = null) {
    return this.has(key) ? this.get(key) : this.set$1(key, typeof defval == 'function' ? defval(key) : defval);
  },
  get$async: async function(key, defval = null) {
    return this.has(key) ? this.get(key) : this.set$1(key, typeof defval == 'function' ? await defval(key) : defval);
  },
  toObject: function() {
